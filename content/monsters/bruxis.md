---
title: Bruxis
tags: [water, projectile, tier1]
---
<div class="monster-card">
  <div class="monster-header">
    <span class="monster-name">Bruxis</span>
    <a href="/tags/tier1" class="monster-tier">Tier 1</a>
  </div>
  <div class="monster-image-wrap">
    <img src="/assets/Portraits/P_Bruxis.PNG" alt="Bruxis" class="monster-img" />
    <div class="monster-types">
      <a href="/tags/water" class="type-badge"><img src="/assets/icons/T_EType_Water.PNG" alt="Water" class="type-icon" /></a>
      <a href="/tags/projectile" class="type-badge"><img src="/assets/icons/T_AType_Projectile.PNG" alt="Projectile" class="type-icon" /></a>
    </div>
  </div>
  <div class="monster-desc">
    Every winter solstice it performs a ritual in which it changes its hat.
  </div>
  <div class="monster-stats-row">
    <div class="stat-pill stat-hp"><span>HP</span><strong>50</strong></div>
    <div class="stat-pill stat-str"><span>STR</span><strong>30</strong></div>
    <div class="stat-pill stat-def"><span>DEF</span><strong>35</strong></div>
    <div class="stat-pill stat-mnd"><span>MND</span><strong>70</strong></div>
    <div class="stat-pill stat-soul"><span>SOUL</span><strong>70</strong></div>
    <div class="stat-pill stat-agi"><span>AGI</span><strong>44</strong></div>
  </div>
  <div class="monster-skills-section">
    <div class="skills-active">
      <h3>Active Skills</h3>
      <table>
        <thead>
          <tr><th>Level</th><th>Skill</th></tr>
        </thead>
        <tbody>
          <tr><td>Base</td><td><a href="/skills/icicle-dart">Icicle Dart</a></td></tr>
          <tr><td>5</td><td><a href="/skills/boreal-gale">Boreal Gale</a></td></tr>
        </tbody>
      </table>
    </div>
    <div class="skills-passive">
      <h3>Passive</h3>
      <p class="passive-block"><a href="/passives/ice-magic"><strong>Ice Magic</strong></a></br> After using a  skill, inflict 1  Freeze to all enemies.</p>
    </div>
  </div>

</div>