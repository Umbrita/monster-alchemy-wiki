---
title: Floro
tags: [nature, strike, tier2]
---
<div class="monster-card">
  <div class="monster-header">
    <span class="monster-name">Floro</span>
    <a href="/tags/tier2" class="monster-tier">Tier 2</a>
  </div>
  <div class="monster-image-wrap">
    <img src="/assets/Portraits/P_Floro.PNG" alt="Floro" class="monster-img" />
    <div class="monster-types">
      <a href="/tags/nature" class="type-badge"><img src="/assets/icons/T_EType_Nature.PNG" alt="Nature" class="type-icon" /></a>
      <a href="/tags/strike" class="type-badge"><img src="/assets/icons/T_AType_Strike.PNG" alt="Strike" class="type-icon" /></a>
    </div>
  </div>
  <div class="monster-desc">
    It eats impressively every three months. Then it sleeps until the next time.
  </div>
  <div class="monster-stats-row">
    <div class="stat-pill stat-hp"><span>HP</span><strong>100</strong></div>
    <div class="stat-pill stat-str"><span>STR</span><strong>60</strong></div>
    <div class="stat-pill stat-def"><span>DEF</span><strong>85</strong></div>
    <div class="stat-pill stat-mnd"><span>MND</span><strong>40</strong></div>
    <div class="stat-pill stat-soul"><span>SOUL</span><strong>50</strong></div>
    <div class="stat-pill stat-agi"><span>AGI</span><strong>45</strong></div>
  </div>
  <div class="monster-skills-section">
    <div class="skills-active">
      <h3>Active Skills</h3>
      <table>
        <thead>
          <tr><th>Level</th><th>Skill</th></tr>
        </thead>
        <tbody>
          <tr><td>Base</td><td><a href="/skills/overwhelming-slam">Overwhelming Slam</a></td></tr>
          <tr><td>5</td><td><a href="/skills/bloom-bash">Bloom Bash</a></td></tr>
        </tbody>
      </table>
    </div>
    <div class="skills-passive">
      <h3>Passive</h3>
      <p class="passive-block"><a href="/passives/healthy-fists"><strong>Healthy Fists</strong></a></br> After using a  attack, gain 1   Regeneration.</p>
    </div>
  </div>

</div>